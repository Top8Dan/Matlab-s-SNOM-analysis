%% image maker, imports in two files, one reflection one absorption and creates an amplitude and phase image.

close all

%folder of reflection
RefFolder = 'C:\Users\k68558dj\OneDrive - The University of Manchester\DataFiles\NPL Measurements\May 2024\May 2024 - updated\NW\Tip2\1250 R';

%folder of Absorption
AbsFolder = 'C:\Users\k68558dj\OneDrive - The University of Manchester\DataFiles\NPL Measurements\May 2024\May 2024 - updated\NW\Tip2\1250 A';

% Import the FIXED data
ReflectionHeight = strcat(RefFolder,'\', '1250 R_Height_PrimaryTrace.txt');

% Import the  MOVING data
AbsorptionHeight = strcat(AbsFolder,'\', '1250 A_Height_PrimaryTrace.txt');

%Do you wanna save??
Save = 'N';

SaveFolder = 'C:\Users\k68558dj\OneDrive - The University of Manchester\DataFiles\NPL Measurements\May 2024\Analysis\NW\RatioNoSubstrateNorm\1250';
...
if RatioNorm =='Y'
    ...
    for n = 1:size(RefData2,1)
            for i = 1:size(RefData2,2)
                ...
                Amp43(n,i) = AmpData4(n,i) / AmpData3(n,i);
                Amp32(n,i) = AmpData3(n,i) / AmpData2(n,i);

                Phase43(n,i) = PhaseData4(n,i) - PhaseData3(n,i);
                Phase32(n,i) = PhaseData3(n,i) - PhaseData2(n,i);
            end
    end
...
% caxis(gca,[0 prctile(AmpData2,99.99,"all")])
...
colorbar
%uncomment if you want colourbars to be automatic
% caxis(gca,[0 prctile(AmpData3,99.99,"all")]) 
clim(gca,[0,20]) %Here you can set the limits of colourbar
title('Amplitude NF3')
...
colorbar
%uncomment if you want colourbars to be automatic
% caxis(gca,[0 prctile(AmpData4,99.99,"all")])
...
if RatioNorm == 'Y'
    figure(7)
        subplot(1,2,1)
        hold on
        Amp32Image = image(Amp32,'CDataMapping','scaled');
        colormap(gca,'hot')
        colorbar
        %uncomment if you want colourbars to be automatic
        clim(gca,[0 prctile(AmpData2,99.99,"all")])
        %clim(gca,[0,18]) %Here you can set the limits of colourbar
        title('Amplitude 3/2')
        Amp32Image.XData = [0 Xlength];
        Amp32Image.YData = [0 Ylength];
        ax = gca;
        xlabel('um')
        ylabel('um')
        ax.YDir = 'reverse';
        xlim(ax,Amp32Image.XData)
        ylim(ax,Amp32Image.YData)
        axis square
        
        
        subplot(1,2,2)
        hold on
        Phase32Image = image(Phase32,'CDataMapping','scaled');
        colormap(gca,'cool')
        colorbar
        title('Phase 3-2')
        Phase32Image.XData = [0 Xlength];
        Phase32Image.YData = [0 Ylength];
        ax = gca;
        xlabel('um')
        ylabel('um')
        ax.YDir = 'reverse';
        xlim(ax,Phase32Image.XData)
        ylim(ax,Phase32Image.YData)
        axis square

     figure(8)
        subplot(1,2,1)
        hold on
        Amp43Image = image(Amp43,'CDataMapping','scaled');
        colormap(gca,'hot')
        colorbar
        %uncomment if you want colourbars to be automatic
        clim(gca,[0 prctile(AmpData2,99.99,"all")])
        %clim(gca,[0,18]) %Here you can set the limits of colourbar
        title('Amplitude 4/3')
        Amp43Image.XData = [0 Xlength];
        Amp43Image.YData = [0 Ylength];
        ax = gca;
        xlabel('um')
        ylabel('um')
        ax.YDir = 'reverse';
        xlim(ax,Amp43Image.XData)
        ylim(ax,Amp43Image.YData)
        axis square
        
        
        subplot(1,2,2)
        hold on
        Phase43Image = image(Phase43,'CDataMapping','scaled');
        colormap(gca,'cool')
        colorbar
        title('Phase 4-3')
        Phase43Image.XData = [0 Xlength];
        Phase43Image.YData = [0 Ylength];
        ax = gca;
        xlabel('um')
        ylabel('um')
        ax.YDir = 'reverse';
        xlim(ax,Phase43Image.XData)
        ylim(ax,Phase43Image.YData)
        axis square
end
...
% saveas(figure(6),strcat(SaveFolder,'\MCT.fig'));
    if RatioNorm == 'Y'
        saveas(figure(7),strcat(SaveFolder,'\32Ratio.fig'));
        saveas(figure(8),strcat(SaveFolder,'\43Ratio.fig'));
    end
end